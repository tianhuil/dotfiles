...
npx ctx7 docs /vercel/next.js "How to set up app router"

# Version-specific
npx ctx7 docs /vercel/next.js/v14.3.0-canary.87 "How to set up app router"
```

The available versions are listed in the `npx ctx7 library` output. Use the closest match to what the user specified.
...
npx ctx7 docs /vercel/next.js "How to add authentication middleware to app router"
...
2. Suggest they authenticate for higher limits: `npx ctx7 login`
...
- Always run `npx ctx7 library` first — `npx ctx7 docs react "hooks"` will fail without a valid ID
