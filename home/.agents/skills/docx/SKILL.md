...
```bash
python SKILL_DIR/scripts/office/validate.py doc.docx
```
...
```bash
python SKILL_DIR/scripts/comment.py unpacked/ 0 "Comment text with &amp; and &#x2019;"
python SKILL_DIR/scripts/comment.py unpacked/ 1 "Reply text" --parent 0
```

### Step 3: Pack
```bash
python SKILL_DIR/scripts/office/pack.py unpacked/ output.docx --original document.docx
```
...
- **Python**: `pip install defusedxml lxml`
