...
- `agent-browser`: vercel labs agent browser, replacement for playwright
...
- `serena`: Semantic code intelligence via `uvx --from git+https://github.com/oraios/serena serena`. Use for symbol-level navigation, refactoring (rename, safe delete), finding references/implementations, replacing symbol bodies, and project memory. Supports 40+ languages. Prefer over text-based search-and-replace for structured code changes.
