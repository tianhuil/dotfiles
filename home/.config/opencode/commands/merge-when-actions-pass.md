...
5. **Wait for Actions**: Watch the workflow run until it completes
6. **Check result**: If the run failed, show failed jobs and stop — the user should fix and re-run this command
...
# Watch workflow run until it finishes
...
### Actions Failed
- If `gh run watch` exits and the run conclusion is not `success`: Show the failed jobs and logs, then "Actions failed. Fix the issues, push, and run this command again." — **STOP**
