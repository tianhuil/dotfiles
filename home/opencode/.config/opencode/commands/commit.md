...
git diff --cached | llm -m gemini-2.5-flash-lite "Generate a short git commit message (1 line, under 72 characters) for the following staged changes generated by \`git diff --cached\`. Output ONLY the commit message, nothing else"
...
git diff --cached
\`\`\`
Please generate a concise, 1-line commit message for these changes."
```
